/**
 * NB UI Kit Shared Page Script
 * - Persists and applies theme (light/dark) across all pages via localStorage
...
;(function () {
  ...
  var SIDEBAR_KEY = 'nb-sidebar-collapsed';
  var saved = null;
  ...
  function injectToggle() {
    ...
    btn.innerHTML =
      '<svg class="nb-theme-fab_sun" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
      ...
      '</svg>' +
      '<svg class="nb-theme-fab_moon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    style.textContent =
      '#nb-theme-fab {' +
        ...
        'background: var(--nb-white); border: var(--nb-border);' +
        'color: var(--nb-gray-600); cursor: pointer;' +
        'display: flex; align-items: center; justify-content: center;' +
        'transition: all 200ms ease; box-shadow: var(--nb-shadow-sm);' +
      '}' +
      '#nb-theme-fab:hover {' +
        'border-color: var(--nb-black); color: var(--nb-black);' +
        'transform: scale(1.08); box-shadow: var(--nb-shadow-md);' +
      '}' +
      '[data-theme="dark"] .nb-theme-fab_sun { display: block; }' +
      '[data-theme="dark"] .nb-theme-fab_moon { display: none; }' +
      '[data-theme="light"] .nb-theme-fab_sun { display: none; }' +
      '[data-theme="light"] .nb-theme-fab_moon { display: block; }' +
      /* Default to light if no attribute */
      '.nb-theme-fab_sun { display: none; }' +
      '.nb-theme-fab_moon { display: block; }';
  ...
  function restoreSidebarState() {
    var sidebar = document.querySelector('.nb-sidebar');
    if (!sidebar) return;
    ...
    var toggleBtn = document.querySelector('.nb-sidebar-toggle');
    if (toggleBtn) {
    ...
    forms.forEach(function(form) {
      ...
      form.addEventListener('submit', function(e) {
        ...
        inputs.forEach(function(input) {
          var error = input.closest('.nb-field') ?
            input.closest('.nb-field').querySelector('.form-error') : null;
          // For password inputs inside nb-password wrapper
          if (!error) {
            var wrapper = input.closest('.nb-password');
            if (wrapper) {
        ...
        if (pw && pwConfirm && pw.value && pwConfirm.value) {
          if (pw.value !== pwConfirm.value) {
            valid = false;
            var confirmError = pwConfirm.closest('.nb-field') ?
              pwConfirm.closest('.nb-field').querySelector('.form-error') : null;
            if (confirmError) {
